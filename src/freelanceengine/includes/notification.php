...
class Fre_Notification extends AE_PostAction
{
    ...
    function admin_report_dispute_project_employer($project_id, $report){
        global $user_ID;
        $project = get_post($project_id);
        if($project->post_author == $user_ID) return;
        // Employer
        $content = 'type=report_dispute_project&project='.$project_id.'&admin='.$user_ID;
    ...
    function update_order($order_id){
        ...
        // check status        
        if(!in_array($order->post_status, array('publish', 'draft'))) return;
    ...
    function fre_notify_item($notify) {
        ...
        if( !in_array($type, array('new_invite', 'approve_order', 'cancel_order', 'approve_withdraw', 'cancel_withdraw'))) {
